    ...
    >
      ...
      >
        ...
        <div className="px-3 pb-4" style={{ minHeight: 320 }}>
          ...
          {screen === 'food' && (
            <>
              <BackBar label="外送點餐" onBack={() => setScreen('home')} />
          ...
          {screen === 'drone' && (
            <>
              <BackBar label="無人機控制" onBack={() => setScreen('home')} />
...
function AppButton({ emoji, label, sublabel, color, borderColor, onClick }: {
...
function BackBar({ label, onBack }: { label: string; onBack: () => void }) {
  return (
    <div className="flex items-center gap-2 mb-1">
      <button onClick={onBack} className="text-white/50 hover:text-white text-xs font-mono">← 返回</button>
